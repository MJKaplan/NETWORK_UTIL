...
#        i+=1
    ...
    chkspawn.append(Checkbutton(master, text="VraySpawner", command = lambda i=i, x=x: togglespawn(conn[i], i, x)))
    spawntest(conn[i], i)
    
    chkburn.append(Checkbutton(master, text="backburner", command = lambda i=i, x=x: toggleburn(conn[i], i, x)))
    burntest(conn[i], i)
